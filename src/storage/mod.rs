...
pub use self::journal::{JournalEntry, JournalRecord, JournalRecordWithCheckSum, JournalSnapshot};
...
mod tests {
    ...
    fn is_put_with(entry: &JournalEntry, id: &LumpId) -> bool {
        if let JournalRecord::Put(id_, _) = entry.record.body() {
            *id_ == *id
        } else {
            false
        }
    ...
    fn is_delete_with(entry: &JournalEntry, id: &LumpId) -> bool {
        if let JournalRecord::Delete(id_) = entry.record.body() {
            id_ == *id
        } else {
            false
        }
